...
class EventsProviderClient:
    ...
    async def _request(self, method: str, endpoint: str, params=None, data=None):
        ...
        if method == "GET":
            response = await self._client.request(method, endpoint, params=params, follow_redirects=True)
        elif method in ["POST", "DELETE"]:
            response = await self._client.request(method, endpoint, json=data, follow_redirects=True)
        else:
        ...
        logger.error(f"Request failed: {method} {endpoint} - {response.status_code} - {error_text}")
    ...
    async def get_events(self, changed_at: str = None, next_url: str = None):
        if next_url:
            response = await self._client.get(next_url, follow_redirects=True)
            return response.json()
        else:
            return await self._request("GET", "/api/events/", params={"changed_at": changed_at})
